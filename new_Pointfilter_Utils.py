...
def parse_arguments():
    ...
    parser.add_argument('--network_model_dir', type=str, default='./Summar2/Models/Train', help='output folder (trained models)')
    ...
    parser.add_argument('--summary_dir', type=str, default='./Summary2/Models/Train/logs', help='tensorboard logs directory')
...
def chamfer_distance(pred_pts, gt_pts):
    gt_kdtree   = cKDTree(gt_pts)
    pred_kdtree = cKDTree(pred_pts)
    
    dist1, _ = gt_kdtree.query(pred_pts)    # pred → GT
    dist2, _ = pred_kdtree.query(gt_pts)    # GT → pred
    
    # ✅ Added 0.5 factor to match paper
    cd = 0.5 * np.mean(dist1**2) + 0.5 * np.mean(dist2**2)
    return cd

def calculate_p2m(denoised_pts, gt_mesh_path):
    ...
    try:
        ...
        query_points = o3d.core.Tensor(denoised_pts, dtype=o3d.core.Dtype.Float32)

        # ✅ Direction 1: denoised points → mesh surface (F2P)
        closest_data   = scene.compute_closest_points(query_points)
        closest_points = closest_data['points'].numpy()
        f2p = 0.5 * np.mean(np.sum((denoised_pts - closest_points)**2, axis=1))

        # ✅ Direction 2: mesh surface → denoised points (P2F)
        sampled = mesh.sample_points_uniformly(number_of_points=len(denoised_pts))
        sampled_pts = np.asarray(sampled.points, dtype=np.float32)
        pred_kdtree = cKDTree(denoised_pts)
        dist_rev, _ = pred_kdtree.query(sampled_pts)
        p2f = 0.5 * np.mean(dist_rev**2)

        return f2p + p2f

    except Exception as e:
        print(f"P2M error for {gt_mesh_path}: {e}")
        return np.nan

# # In new_Pointfilter_Utils.py, replace the old function with this one

# def calculate_p2m(denoised_pts, gt_mesh_path):
#     """
#     Calculates the average Point-to-Mesh distance.
    
#     Args:
#         denoised_pts (np.ndarray): The denoised point cloud.
#         gt_mesh_path (str): Path to the ground truth mesh file in .off format.
#     """
#     # Load the ground truth mesh from the .off file
#     gt_mesh = o3d.io.read_triangle_mesh(gt_mesh_path)
    
#     if not gt_mesh.has_vertices() or not gt_mesh.has_triangles():
#         print(f"Warning: Mesh not found or invalid at {gt_mesh_path}")
#         return np.nan
        
#     # Create an Open3D point cloud from the denoised points
#     denoised_pcd = o3d.geometry.PointCloud()
#     denoised_pcd.points = o3d.utility.Vector3dVector(denoised_pts)
    
#     # --- THIS IS THE FIX ---
#     # The method is called on the point cloud, not the mesh.
#     distances = denoised_pcd.compute_point_cloud_distance(gt_mesh)
#     # --- END OF FIX ---
    
#     distances = np.asarray(distances)
    
#     # Return the mean of the squared distances for consistency with CD
...
def add_uniform_noise(points, noise_level=0.02):
    noise = (torch.rand_like(points) * 2.0 - 1.0) * noise_level
    return points + noise

def add_sparse_noise(points, noise_level=0.02, corruption_rate=0.1):
    if corruption_rate <= 0:
        return points

    noisy_points = points.clone()
    num_points = noisy_points.shape[0]
    num_corrupted = min(num_points, max(1, int(num_points * corruption_rate)))
    indices = torch.randperm(num_points, device=noisy_points.device)[:num_corrupted]
    sparse_noise = torch.randn((num_corrupted, noisy_points.shape[1]), device=noisy_points.device) * noise_level
    noisy_points[indices] = noisy_points[indices] + sparse_noise
    return noisy_points

def add_mixed_noise(points, noise_level=0.02, corruption_rate=0.1):
    noisy_points = add_gaussian_noise(points, noise_level * 0.5)
    noisy_points = add_uniform_noise(noisy_points, noise_level * 0.5)
    noisy_points = add_sparse_noise(noisy_points, noise_level, corruption_rate)
    return noisy_points
...
def add_noise_to_batch(batch, noise_type='none', noise_level=0.02, corruption_rate=0.1):
    if noise_type == 'none':
        ...
        return add_uniform_noise(batch, noise_level)
    elif noise_type == 'sparse':
        return add_sparse_noise(batch, noise_level, corruption_rate)
    elif noise_type == 'mixed':
        return add_mixed_noise(batch, noise_level, corruption_rate)
...
def random_rotate_pointcloud(points):
    ...
    rotated = np.dot(points.detach().cpu().numpy(), rotation_matrix.T)
    return torch.from_numpy(rotated).to(device=points.device, dtype=points.dtype)
